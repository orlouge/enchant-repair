...
public class Config {
    ...
    public static void load() {
        ...
        defaultProps.setProperty("anvil_repair_extra_items",
                REPAIR_EXTRA_ITEMS.get().entrySet().stream().map(
                        entry -> Registries.ITEM.getId(entry.getKey()) + "+" + entry.getValue().stream().map(i -> Registries.ITEM.getId(i) + "").collect(Collectors.joining("+"))
        ...
        if (f.isFile() && f.canRead()) {
            try (FileInputStream in = new FileInputStream(f)) {
                ...
                REPAIR_EXTRA_ITEMS = new Lazy<>(() -> Arrays.stream(anvilRepairExtraItems).map(entry -> {
                    ...
                    return Map.entry(Registries.ITEM.get(new Identifier(splitEntry[0])), items);
